...
uint16_t mac1;
uint16_t mac2;
std::u16string licenseFilePath = u"C:\\Users\\user\\source\\repos\\SuperKeygen64\\SuperKeygen64\\key.txt";
...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
                     ...
                     _In_ int       nCmdShow)
{
    // Убираем предупреждение, тк не используем эти переменные
   ...
   std::thread clientThread([hWnd]()
       {
           ...
           if (!LicenseCheck())
           {
               licenseMessage = "Key not found or expired, please send your HWID by email\n Your HWID: ";
               licenseMessage += machineHash();
               hwid = std::wstring(licenseMessage.begin(), licenseMessage.end());
               MessageBox(hWnd, (LPCWSTR)hwid.c_str(), __TEXT("License Notification"), MB_OK | MB_ICONINFORMATION);
               {
           ...
           else
           {
               ...
               if (decryptedInfoJ["licenseType"] == 0)
                   licenseType = "Free";
               else if (decryptedInfoJ["licenseType"] == 1)
                   licenseType = "Home";
               else if (decryptedInfoJ["licenseType"] == 2)
                   licenseType = "Pro";
               long long startDay = decryptedInfoJ["startDay"];
               long long expirationTime = decryptedInfoJ["expirationTime"];
               time_t remainingTimeT = startDay + expirationTime;
               tm expirationDate;
               localtime_s(&expirationDate, &remainingTimeT);
               licenseMessage = std::format("Antivirus launched\nLicense is active\nLicense Type: {}\nExpiration Date: {}.{}.{}", licenseType,
                   expirationDate.tm_mday, expirationDate.tm_mon + 1, expirationDate.tm_year + 1900);
               std::wstring licenseMessageNew = std::wstring(licenseMessage.begin(), licenseMessage.end());
               MessageBox(hWnd, (LPCWSTR)licenseMessageNew.c_str(), __TEXT("License Notification"), MB_OK | MB_ICONINFORMATION);
           }
           while (true) {
               // Ждем новой команды
               {
    ...
    if (licenseKey.is_open())
    {
        while (std::getline(licenseKey, encryptedKey))
        {
            if ((encryptedKey.length() & 3) != 0)
            {
                ...
                MessageBox(NULL, L"Base64 size error", L"Error message", MB_OK);
                return FALSE;
            }
            decryptedInfo = base64::from_base64(xor_operation(base64::from_base64(encryptedKey)));
        }
        decryptedInfoJ = json::parse(decryptedInfo);
